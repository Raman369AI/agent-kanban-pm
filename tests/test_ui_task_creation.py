...
def run_test():
    ...
    # Step 1: Resolve bootstrapped local human owner
    print("\n1. Resolve local human owner...")
    status, body = fetch(f"{base}/entities/me")
    human = json.loads(body)
    human_id = human["id"]
    print(f"   ✅ Local owner: id={human_id}, role={human['role']}")
