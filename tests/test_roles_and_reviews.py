...
class TestApprovalQueueAPI:
    def test_approval_queue_requires_auth_and_manager_resolution(self):
        ...
        with TestClient(app) as client:
            ...
            worker_r = client.post("/entities/register/agent", json={
...
class TestAssignmentLauncher:
        ...
        async def run_case():
            ...
            try:
                async with session_maker() as db_session:
                    ...
                    agent = Entity(name="worker-agent", entity_type=EntityType.AGENT, role=Role.WORKER, is_active=True)
                    ...
                    todo_stage = Stage(name="To Do", description="", order=1, project_id=project.id)
                    in_progress_stage = Stage(name="In Progress", description="", order=2, project_id=project.id)
                    db_session.add_all([todo_stage, in_progress_stage])
                    await db_session.flush()
                    ...
                    event = await AssignmentLauncher()._mark_task_started(db_session, task, agent)

                    assert task.status == TaskStatus.IN_PROGRESS
                    assert task.stage_id == in_progress_stage.id
                    assert event["stage_changed"] is True
                    assert event["from_stage_id"] == todo_stage.id
                    assert event["to_stage_id"] == in_progress_stage.id
            finally:
                async with engine.begin() as conn:
                    await conn.run_sync(Base.metadata.drop_all)
                await engine.dispose()
    ...
    def test_build_agent_command_for_installed_cli_shapes(self, monkeypatch):
        ...
        codex = AdapterSpec(
            ...
            task_command=TaskCommandSpec(args=["--ask-for-approval", "on-request", "-C", "{workspace}", "{prompt}"]),
        )
        ...
        opencode = AdapterSpec(
            ...
            task_command=TaskCommandSpec(args=["run", "{prompt}"]),
        )
    ...
    def test_role_supervisor_model_flag_is_adapter_data(self, monkeypatch):
        ...
        adapter = AdapterSpec(
            ...
            invoke=InvokeSpec(command="custom-agent", model_flag="--model-name"),
        )
        assignment = RoleAssignment(agent="custom-agent", model="large", mode="headless")
