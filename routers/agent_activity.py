...
from models import (
    AgentHeartbeat, AgentActivity, AgentSession, Entity, Task, Project,
    AgentStatusType, ActivityType, AgentSessionStatus, ProjectWorkspace,
    OrchestrationDecision, TaskLease, ActivitySummary, UserContribution,
...
async def sync_github_contributions(
    ...
    if not _is_git_pr_role(current_entity):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the assigned Git PR role may sync GitHub contributions. "
                   "Override in preferences if needed.",
        )
...
async def list_agent_approvals(
    ...
    has_explicit_identity = bool(request.headers.get("x-api-key") or request.headers.get("x-entity-id"))
