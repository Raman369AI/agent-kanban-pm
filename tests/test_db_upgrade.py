...
# Columns added by migrations v1-v19, keyed by table. Dropping these from a
...
MIGRATED_COLUMNS = {
    ...
    "diff_reviews": ["work_revision", "base_revision", "diff_sha256"],
    ...
    "agent_sessions": [
        ...
        "handoff_artifacts_json", "review_base_revision",
    ],
}
...
def test_upgrade_records_the_migration_chain(legacy_db):
    ...
    assert versions >= set(range(1, 20)), f"migration chain incomplete: {sorted(versions)}"
...
def test_fresh_database_also_records_the_chain(tmp_path):
    ...
    assert versions >= set(range(1, 20)), (
