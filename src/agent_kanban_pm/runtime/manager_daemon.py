...
NOTE: The role supervisor (agent_kanban_pm.runtime.role_supervisor) is now the
...
from agent_kanban_pm.runtime.preferences import load_preferences, PREFERENCES_PATH
    ...
    config = {
        "mcpServers": {
            "kanban": {
                ...
                "args": ["-m", "agent_kanban_pm.mcp.server"],
                "env": {
...
def start_manager_daemon():
    ...
    prefs = load_preferences()
    if not prefs:
        logger.error(f"No preferences at {PREFERENCES_PATH}. Run: kanban init")
        sys.exit(1)
