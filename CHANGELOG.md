...
- **Adapter flags match what each CLI actually accepts** — audited against the installed binaries. `agy --print` takes the prompt as its value, so `--print --add-dir <dir> <prompt>` made it read `--add-dir` as the prompt and ignore the real one; the prompt is now attached as `--print={prompt}`. `claude --add-dir` is variadic and swallowed the prompt as a second directory, so a flag now terminates it before the positional prompt. `codex --full-auto` no longer exists and exits with `unexpected argument`; the bypass flag is `--dangerously-bypass-approvals-and-sandbox`. opencode and aider were already correct.
