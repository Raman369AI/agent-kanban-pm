...
Usage:
    kanban init               # Interactive setup wizard
    kanban agents list        # Show installed adapters
    kanban daemon start       # Start manager daemon
    kanban daemon status      # Check daemon status
    kanban daemon stop        # Stop manager daemon
    kanban run                # Start server + UI + supervisor + agents
    kanban roles list         # Show role assignments
    kanban roles assign <role> <agent>  # Assign agent to role
    kanban roles start        # Start all role agents
    kanban roles stop         # Stop all role agents
    kanban roles status       # Show role session status
"""
...
def cmd_init(args):
    ...
    print("Or start just the daemon: kanban daemon start")
...
def cmd_daemon(args):
    from agent_kanban_pm.runtime.manager_daemon import start_manager_daemon
    start_manager_daemon()


def cmd_daemon_status(args):
    from agent_kanban_pm.runtime.manager_daemon import daemon_status
...
def cmd_daemon_stop(args):
    from agent_kanban_pm.runtime.manager_daemon import daemon_stop
    result = daemon_stop()
...
def cmd_run(args):
    ...
    server_proc = subprocess.Popen(
        [sys.executable, "-m", "uvicorn", "agent_kanban_pm.app:app", "--host", host, "--port", str(port)],
        stdout=sys.stdout,
        stderr=sys.stderr,
        env=env,
    )
    ...
    if not no_supervisor:
        from agent_kanban_pm.runtime.role_supervisor import RoleSupervisor
...
def cmd_roles_list(args):
    ...
    if not prefs:
        print("No preferences found. Run: kanban init")
        return
...
def cmd_roles_assign(args):
    ...
    if not adapter:
        ...
        if not shutil.which(command):
            ...
            print("Use 'kanban agents discover' to see common installed CLIs.")
            return
...
def cmd_sheet(args):
    ...
    from agent_kanban_pm.models import Entity, EntityType, Project, Task, task_assignments
...
def _default_port():
    from agent_kanban_pm.runtime.instance import get_port
    return get_port()


def _default_api_base(port=None):
    from agent_kanban_pm.runtime.instance import get_api_base
    ...
    for role_name, assignment in assignments.items():
        ...
        if session:
            ...
            if session.tmux_session:
                from agent_kanban_pm.runtime.role_supervisor import tmux_is_running
...
def main():
    ...
    def _cmd_chat(args):
        from agent_kanban_pm.cli.chat import cmd_chat as _run
        _run(args)
