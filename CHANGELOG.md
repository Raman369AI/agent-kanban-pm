...
- **Two-Tier RBAC** — Enforceable role-based access control:
  - `Role` enum: `OWNER`, `MANAGER`, `WORKER`, `VIEWER`
  - `Scope` enum (`OWNER`, `MANAGER`, `WORKER`, `READONLY`) tied to API key, distinct from `Role`
  - `get_effective_role()` uses the more restrictive of `role` vs `scope`
  - `X-API-Key` and `X-Entity-ID` header support
...
- **Database Migrations** — `_migrate_db_schema()` adds missing columns (`role`, `scope`, `created_by`, `version`) and backfills existing data
...
- **MCP auth** — MCP server now reads `KANBAN_AGENT_NAME` from env and looks up `Entity.name`. No Kanban-issued API key needed in local-first mode.
...
- **`~/.kanban/manager_key`** — Kanban-issued API key file removed. Provider env vars are the only secrets.
...
- **Agent Inbox** — `agent_inbox.py` terminal notifier daemon with `--auto-ack` mode
