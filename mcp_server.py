...
class KanbanMCPServer:
    ...
    def _resolve_caller(self):
        ...
        Local-first: the OS process is the trust boundary. No Kanban-issued
        API key is needed; provider keys (ANTHROPIC_API_KEY etc.) are
    ...
    async def _handle_list_agents(self, args: dict) -> list:
        ...
        async with async_session_maker() as db:
            ...
            return [
                {"id": a.id, "name": a.name, "skills": a.skills, "api_key_prefix": a.api_key[:8] + "..." if a.api_key else None}
                for a in agents
            ]
